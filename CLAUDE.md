...
- **Every shipped fix or feature MUST update CHANGELOG.md — run `/gstack-document-release` to do it.** Before (or as part of) pushing a user-facing change to `main`, add a bullet under `## [Unreleased]` in the matching `Added`/`Changed`/`Fixed`/`Removed` group. `/gstack-document-release` cross-references the diff, writes the entry in the project's changelog voice (user-impact first, not "what the code does"), and syncs the other docs. **Promoting `[Unreleased]` into a dated, versioned section is manual** — `release.yml` never touches CHANGELOG.md (it only bumps `tauri.conf.json` + `Cargo.toml` and syncs those back), and the GitHub release notes are generated from commit subjects, not from the changelog. So after a release publishes, rename `[Unreleased]` to `## [X.Y.Z] - YYYY-MM-DD` and push, or the shipped version has no changelog section. A change is not "shipped" until its changelog entry exists.
